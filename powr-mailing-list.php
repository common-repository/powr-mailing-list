<?php
    ...
    Plugin URI: https://www.powr.io/plugins/mailing-list
    Description: Grow your mailing list with zero effort  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    class Powr_Mailing_List extends WP_Widget{
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Mailing List.
        </p>
        <p>
    ...
    function powr_mailing_list_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-mailing-list' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-mailing-list' label='$label'></div>";
      }else{
      	"<div class='powr-mailing-list'></div>";
      }
    }
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_mailing_list_menu' );
    function powr_mailing_list_menu() {
      add_menu_page( 'POWr Mailing List', 'POWr Mailing List', 'manage_options', 'powr-mailing-list-settings', 'powr_mailing_list_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_mailing_list_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/mailing-list';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/mailing-list';
      }
    ...
    register_activation_hook(__FILE__, 'powr_mailing_list_plugin_activate');
      add_action('admin_init', 'powr_mailing_list_plugin_redirect');

      function powr_mailing_list_plugin_activate() {
      add_option('powr_mailing_list_plugin_do_activation_redirect', true);
      }
      ...
      function powr_mailing_list_plugin_redirect() {
      if (get_option('powr_mailing_list_plugin_do_activation_redirect', false)) {
          delete_option('powr_mailing_list_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-mailing-list-settings&' );
          }
       }
      }
